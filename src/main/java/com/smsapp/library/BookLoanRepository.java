...
public interface BookLoanRepository extends JpaRepository<BookLoan, UUID> {
    ...
    @Query("select l.id as id, l.bookId as bookId, b.title as bookTitle, b.author as bookAuthor, "
            ...
            + "where l.bookId = b.id and l.studentId = :studentId "
            + "order by l.issuedDate desc, l.id")
    List<LoanWithBook> findLoanHistory(@Param("studentId") UUID studentId);

    /** Every loan that has not been returned yet, soonest due first, with book + student joined in. */
    @Query("select l.id as id, l.bookId as bookId, b.title as bookTitle, "
            ...
            + "where l.bookId = b.id and l.studentId = s.id and l.returnedDate is null "
            + "order by l.dueDate asc, l.id")
    List<ActiveLoan> findActiveLoans();
