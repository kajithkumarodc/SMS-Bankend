...
/**
 * Library catalog + issue/return (plan section 2). {@code available_copies} is
 * kept in step with the loans here: -1 on issue, +1 on return.
 */
@Service
public class LibraryService {

    /** Standard loan period. Configurable is a later slice. */
    ...
    @Transactional
    public LibraryBook addBook(CreateBookRequest request) {
    ...
    public Page<LibraryBook> listBooks(String q, Pageable pageable) {
        if (q == null || q.isBlank()) {
            return bookRepository.findAll(pageable);
        }
        return bookRepository.search(q.trim(), pageable);
    }

    // --- Issue / return ------------------------------------------

    /**
     * @throws ApiException 404 if the book or the student does not exist, 400 if
     *         no copies are available.
     */
    @Transactional
    public BookLoan issue(IssueLoanRequest request) {
    ...
    /**
     * @throws ApiException 404 if the loan does not exist, 409 if it has already
     *         been returned.
     */
    @Transactional
    public BookLoan returnLoan(UUID loanId) {
     ...
     */
    @Transactional(readOnly = true)
    public List<LoanWithBook> loanHistoryForStudent(UUID studentId) {
        if (studentRepository.findById(studentId).isEmpty()) {
            throw new ApiException("Student not found", HttpStatus.NOT_FOUND);
        }
        return loanRepository.findLoanHistory(studentId);
    }

    /** Portal use: the caller's ownership of {@code studentId} has already been proven. */
    @Transactional(readOnly = true)
    public List<LoanWithBook> loanHistoryForOwnedStudent(UUID studentId) {
        return loanRepository.findLoanHistory(studentId);
    }

    /** Every not-yet-returned loan, for the staff "active loans" view. */
    @Transactional(readOnly = true)
    public List<ActiveLoan> activeLoans() {
